...
func scanServer(rows interface {
	...
	err := rows.Scan(&s.ID, &s.UserID, &s.GroupID, &s.Name, &s.ServerKey, &s.Type, &s.OS,
		&s.Status, &s.OnMap, &s.Lat, &s.Lng, &s.Hostname, &s.AgentVersion, &s.IntervalSeconds, &s.LastSeen, &s.CreatedAt)
	return s, err
}

const serverCols = `id, user_id, group_id, name, server_key, type, os, status, on_map, lat, lng, hostname, agent_version, interval_seconds, last_seen, created_at`
...
func AgentReceive(c *gin.Context) {
	...
	// Update server metadata. interval_seconds is only persisted when the
	// agent reports a positive value (older agents may omit the field).
	if payload.IntervalSeconds > 0 {
		_, _ = db.DB.Exec(`UPDATE servers SET last_seen = datetime('now'), status = 'online',
			hostname = ?, os = ?, agent_version = ?, interval_seconds = ? WHERE id = ?`,
			payload.Hostname, payload.OS, payload.AgentVersion, payload.IntervalSeconds, serverID)
	} else {
		_, _ = db.DB.Exec(`UPDATE servers SET last_seen = datetime('now'), status = 'online',
...
func upsertServices(serverID int64, payload models.AgentPayload) {
	...
	// Remove services no longer reported by the agent.
	if len(seen) == 0 {
		// Agent reported no services at all — clear everything.
		_, _ = db.DB.Exec(`DELETE FROM server_services WHERE server_id = ?`, serverID)
	} else {
		// Delete entries not in the current payload.
		rows, err := db.DB.Query(`SELECT id, name FROM server_services WHERE server_id = ?`, serverID)
		if err == nil {
			defer rows.Close()
			var toDelete []int64
			for rows.Next() {
			...
			for _, id := range toDelete {
				_, _ = db.DB.Exec(`DELETE FROM server_services WHERE id = ?`, id)
			}
		}
	}
}
